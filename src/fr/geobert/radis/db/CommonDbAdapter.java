...
public class CommonDbAdapter {
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			...
			default:
				...
				if (null != c) {
					if (c.moveToFirst()) {
						do {
							CommonDbAdapter.this.consolidateSums(c.getLong(0),
									db);
						} while (c.moveToNext());
					}
					c.close();
	...
	public long createAccount(String name, String desc, long start_sum,
			String currency, int projectionMode, String projectionDate)
		...
		setCurrentSumAndDate(0, initialValues, start_sum, projectionMode,
				projectionDate);
	...
	private void setCurrentSumAndDate(long accountId, ContentValues values,
			final long start_sum, final int projectionMode,
			final String projectionDate) throws ParseException {
		Log.d("Radis", "setCurrentSumAndDate start_sum = " + start_sum
	...
	public Cursor fetchAllAccounts() {
		Cursor c = mDb.query(DATABASE_ACCOUNT_TABLE, new String[] {
				...
				KEY_ACCOUNT_PROJECTION_MODE, KEY_ACCOUNT_PROJECTION_DATE },
				null, null, null, null, null);
		if (c != null) {
			c.moveToFirst();
		}
	...
	public Cursor fetchAccount(long accountId) throws SQLException {
		Cursor c = mDb.query(true, DATABASE_ACCOUNT_TABLE, new String[] {
	...
	public boolean updateAccountProjectionDate(long accountId,
		...
		if (projectionController.hasChanged()) {
			updateAccountProjectionDate(accountId,
	...
	private boolean updateAccountProjectionDate(long accountId,
			final int projMode, final String projDate) throws ParseException {
	...
	public boolean updateAccountProjectionDate(long accountId)
		...
		if (null != c) {
			if (c.moveToFirst()) {
				res = updateAccountProjectionDate(accountId, c.getInt(c
						.getColumnIndex(KEY_ACCOUNT_PROJECTION_MODE)), c
						.getString(c
	...
	public boolean updateAccount(long accountId, String name, String desc,
		...
		setCurrentSumAndDate(accountId, args, start_sum,
	...
	public Cursor fetchOpOfMonth(final GregorianCalendar date,
	...
	public Cursor fetchOpBetweenDate(final GregorianCalendar today,
			final GregorianCalendar latest, final long accountId) {
		Cursor c = null;
	...
	public void updateOp(final long opId, final long schOpId) {
		...
		mDb.update(DATABASE_OPERATIONS_TABLE, args, KEY_OP_ROWID + "=" + opId,
				null);
	}
	...
	public void setPref(final String key, final String value) {
		...
		if (null == getPref(key)) {
			...
			// update
			mDb.update(DATABASE_PREFS_TABLE, values, KEY_PREFS_NAME + "='"
					+ key + "'", null);
		}
	}
	...
	public void deletePref(final String key) {
		mDb.delete(DATABASE_PREFS_TABLE, KEY_PREFS_NAME + "='" + key + "'",
				null);
	}
					...
					try {
						setCurrentSumAndDate(
								...
								account.getString(account
										.getColumnIndex(KEY_ACCOUNT_PROJECTION_DATE)));
						mDb.update(DATABASE_ACCOUNT_TABLE, values,
