...
public class QuickAddController {
	...
	public QuickAddController(Activity activity, QuickAddInterface protocol) {
		...
		mProtocol = protocol;
		mQuickAddThirdParty = (MyAutoCompleteTextView) activity.findViewById(R.id.quickadd_third_party);
		mQuickAddAmount = (EditText) activity.findViewById(R.id.quickadd_amount);
		mQuickAddButton = (Button) activity.findViewById(R.id.quickadd_validate);
	...
	private void quickAddOp() throws Exception {
		...
		assert(mAccountId != 0);
		mDbHelper.createOp(op, mAccountId);

		RadisService.updateAccountSum(op.mSum, mAccountId, op.getDate(), mDbHelper);
